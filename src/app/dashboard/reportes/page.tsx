...
export default function ReportesPage() {
  ...
  const loadEstudiantes = async () => {
    ...
    const res = await fetch(`${API}/estudiantes?limit=100`, { headers: { Authorization: `Bearer ${token}` } });
    if (!res.ok) return;
  ...
  const loadGrupos = async () => {
    const token = getAuthToken();
    const res = await fetch(`${API}/grupos`, { headers: { Authorization: `Bearer ${token}` } });
    if (!res.ok) return;
    const data = await res.json();
  ...
  const loadDashboardStats = async () => {
    const token = getAuthToken();
    const res = await fetch(`${API}/reportes/dashboard/stats`, { headers: { Authorization: `Bearer ${token}` } });
    if (!res.ok) return;
    const data = await res.json();
  ...
  const loadGrupoStats = async (grupoId: string) => {
    const token = getAuthToken();
    try {
      const res = await fetch(`${API}/reportes/grupo/${grupoId}/estadisticas`, { headers: { Authorization: `Bearer ${token}` } });
      if (!res.ok) return;
      const data = await res.json();
          ...
          {dashboardStats && (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '20px', marginTop: '20px' }}>
              <div style={{ padding: '20px', background: '#3b82f6', color: 'white', borderRadius: '8px' }}>
                <div style={{ fontSize: '32px', fontWeight: 'bold' }}>{dashboardStats.total_estudiantes || 0}</div>
                <div>Estudiantes</div>
              </div>
              <div style={{ padding: '20px', background: '#22c55e', color: 'white', borderRadius: '8px' }}>
                <div style={{ fontSize: '32px', fontWeight: 'bold' }}>{dashboardStats.total_empleados || 0}</div>
                <div>Empleados</div>
              </div>
              <div style={{ padding: '20px', background: '#f59e0b', color: 'white', borderRadius: '8px' }}>
                <div style={{ fontSize: '32px', fontWeight: 'bold' }}>{dashboardStats.total_matriculas || 0}</div>
                <div>Matrículas</div>
              </div>
              <div style={{ padding: '20px', background: '#8b5cf6', color: 'white', borderRadius: '8px' }}>
                <div style={{ fontSize: '32px', fontWeight: 'bold' }}>{dashboardStats.promedio_asistencia?.toFixed(1) || 0}%</div>
        ...
        <div>
          ...
          <div style={{ display: 'flex', gap: '10px', marginBottom: '20px' }}>
            <select
              ...
              onChange={e => { setEstudianteSeleccionado(e.target.value); loadBoletin(e.target.value); }}
          ...
          {boletin && (
            <div style={{ background: 'white', padding: '20px', borderRadius: '8px', border: '1px solid #e5e7eb' }}>
              <h4 style={{ marginBottom: '15px' }}>
                {boletin.estudiante?.primer_nombre} {boletin.estudiante?.primer_apellido}
              </h4>
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr style={{ background: '#f3f4f6' }}>
                    ...
                    <th style={{ padding: '10px', textAlign: 'center' }}>Desempeño</th>
                    <th style={{ padding: '10px', textAlign: 'left' }}>Observación</th>
                  </tr>
                </thead>
                <tbody>
                  {(boletin.calificaciones || boletin.notas || []).map((nota: any, idx: number) => (
                    <tr key={idx} style={{ borderBottom: '1px solid #e5e7eb' }}>
                      <td style={{ padding: '10px' }}>{nota.asignatura?.nombre || nota.asignatura || '-'}</td>
                      <td style={{ padding: '10px', textAlign: 'center' }}>{nota.nota_final?.toFixed(1) || '-'}</td>
                      ...
                      <td style={{ padding: '10px' }}>{nota.observacion_docente || '-'}</td>
                    </tr>
                  ))}
                  {(boletin.calificaciones?.length === 0 && boletin.notas?.length === 0) && (
                    <tr><td colSpan={4} style={{ padding: '20px', textAlign: 'center' }}>No hay calificaciones registradas</td></tr>
                  )}
                </tbody>
              </table>
            </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ background: '#f3f4f6' }}>
                ...
                <th style={{ padding: '10px', textAlign: 'center' }}>Estudiantes</th>
                <th style={{ padding: '10px', textAlign: 'center' }}>Promedio Notas</th>
