...
export default function EvaluacionPage() {
  ...
  const loadActividades = async () => {
    ...
    const res = await fetch(`${API}/evaluacion/actividades?${params}`, {
  ...
  const loadNotasPeriodo = async () => {
    ...
    const res = await fetch(`${API}/evaluacion/notas-periodo?${params}`, {
      headers: { Authorization: `Bearer ${token}` },
    });
    const data = await res.json();
  ...
  const handleDeleteActividad = async (id: string) => {
    ...
    const token = getAuthToken();
    await fetch(`${API}/evaluacion/actividades/${id}`, {
      method: "DELETE",
      headers: { Authorization: `Bearer ${token}` },
    });
    loadActividades();
  ...
  const handleDeleteBloque = async (id: string) => {
    ...
    const token = getAuthToken();
    await fetch(`${API}/evaluacion/bloques-horarios/${id}`, {
      method: "DELETE",
      headers: { Authorization: `Bearer ${token}` },
    });
    loadBloques();
  ...
  const handleDeleteNota = async (id: string) => {
    ...
    const token = getAuthToken();
    await fetch(`${API}/evaluacion/notas-periodo/${id}`, {
      method: "DELETE",
      headers: { Authorization: `Bearer ${token}` },
    });
    loadNotasPeriodo();
            ...
            <tbody>
              {bloques.map((bloque) => (
                ...
                >
                  ...
                  <td style={{ padding: "10px" }}>
                    {bloque.asignacion?.asignatura?.nombre || "-"}
                  </td>
                  <td style={{ padding: "10px" }}>
                    {bloque.asignacion?.grupo?.nombre || "-"}
                  </td>
                  <td style={{ padding: "10px" }}>
                    <button
            ...
            <tbody>
              {notasPeriodo.map((nota) => (
                <tr key={nota.id} style={{ borderBottom: "1px solid #e5e7eb" }}>
                  <td style={{ padding: "10px" }}>
                    {nota.estudiante?.primer_nombre}{" "}
                    {nota.estudiante?.primer_apellido}
                  </td>
                  <td style={{ padding: "10px" }}>
                    {nota.estudiante?.numero_documento}
                  </td>
