    ...
    const fetchData = async () => {
      try {
        ...
        const [resStats, resFact] = await Promise.all([
          ...
          fetch(`${API_URL}/financiero/facturas`, {
            headers: { Authorization: `Bearer ${token}` },
          }),
        ]);
        ...
        <div className={styles.statsGrid}>
          <div className={styles.statCard}>
            ...
            <span className={styles.statValue}>
              ${stats.total_facturado?.toLocaleString()}
            </span>
          </div>
          <div className={styles.statCard}>
          ...
          <div className={styles.statCard}>
            ...
            <span className={styles.statValue + " " + styles.pending}>
              ${stats.total_pendiente?.toLocaleString()}
            </span>
          </div>
        </div>
      )}
              ...
              <tbody>
                {facturas.map((fac) => (
                  <tr key={fac.id}>
                    ...
                    <td>
                      {fac.estudiante?.primer_nombre}{" "}
                      {fac.estudiante?.primer_apellido}
                    </td>
                    ...
                    <td>
                      <span
                        className={`${styles.badge} ${styles[fac.estado?.toLowerCase()]}`}
                      >
                        {fac.estado}
                      </span>
                    </td>
