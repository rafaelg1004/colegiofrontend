...
export const AsistenciaForm = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setAsignaturas(Array.isArray(areas) ? areas.flatMap((a: any) => a.asignatura || []) : []);
    };
    fetchData();
  }, []);
  ...
  const handleCargarLista = async () => {
    ...
    try {
      ...
      if (dataExistente.length > 0) {
        setEstudiantes(dataExistente.map((a: any) => ({
          ...a.estudiante,
          asistencia_id: a.id,
          estado: a.estado,
        ...
        setEstudiantes(dataEst.map((e: any) => ({
          ...e.estudiante,
          estado: 'Presente',
          justificacion: ''
        })));
  ...
  const updateEstado = (estudianteId: string, nuevoEstado: string) => {
    setEstudiantes(prev => prev.map(e => e.id === estudianteId ? { ...e, estado: nuevoEstado } : e));
  };
    ...
    <div className={styles.container}>
      <header className={styles.header}>
        ...
        <div className={styles.filters}>
          ...
          <button onClick={handleCargarLista} className={styles.loadBtn} disabled={loading}>
            {loading ? '...' : 'Cargar'}
          </button>
        </div>
      </header>
      ...
      {estudiantes.length > 0 && (
        <div className={styles.listContainer}>
          <div className={styles.stats}>
            ...
            <span className={styles.present}>Presentes: {estudiantes.filter(e => e.estado === 'Presente').length}</span>
            <span className={styles.absent}>Ausentes: {estudiantes.filter(e => e.estado === 'Ausente').length}</span>
          </div>
            ...
            {estudiantes.map((est) => (
              <div key={est.id} className={styles.studentItem}>
                <div className={styles.studentInfo}>
                  <p className={styles.studentName}>{est.primer_nombre} {est.primer_apellido}</p>
          ...
          <div className={styles.footer}>
            <button onClick={handleGuardar} className={styles.saveBtn} disabled={saving}>
